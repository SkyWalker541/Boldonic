...
local BoldonicToastWidget = InputContainer:extend{
    text = "",
    timeout = 3,
    dismissable = true,
}

function BoldonicToastWidget:init()
...
function BoldonicToastWidget:onShow()
...
function BoldonicToastWidget:onCloseWidget()
    if self._timer then
...
function BoldonicToastWidget:onAnyKeyPressed()
    if self.dismissable ~= false then
        self:close()
        return true
    end
...
function BoldonicToastWidget:onTapDismiss()
    if self.dismissable ~= false then
        self:close()
        return true
    end
...
function BoldonicToastWidget:close()
    if self._timer then
        UIManager:unschedule(self._timer)
        self._timer = nil
    end
...
function BoldonicToast.show(text, timeout)
    local toast = BoldonicToastWidget:new{
        text = text or "",
